...
def main(params):
	...
	# make directories.
	utils.refresh_directory('data/save')

	# parameter settings.
	npts = 4
	offset = int(sampleSize/8.0)
	...
	for iter in range(tpslen):
		...
		utils.printProgress(iter, tpslen, 'Progress:', 'Complete', 1, 50)
	
	# save to h5 file.
	f = h5py.File('data/save/tps.h5', "w")
	f.create_dataset("_wfx", dtype='float32', data=g_mat_x)
	f.create_dataset("_wfy", dtype='float32', data=g_mat_y)
	print '\nwrote data/save/tps.h5'
